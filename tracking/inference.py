...
class InferenceModule:
    ...
    def getObservationProb(
        ...
        pacman_ghost_distance = manhattanDistance(pacmanPosition, ghostPosition)
        return busters.getObservationProbability(noisyDistance, pacman_ghost_distance)
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for pos in self.allPositions:
            prior_belief = self.beliefs[pos]
            prob_observation = self.getObservationProb(
                observation, pacman_position, pos, jail_position
            )
            updated_beliefs[pos] = prior_belief * prob_observation
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for oldPos in self.allPositions:
            ...
            for newPos, prob in new_pos_dist.items():
                updated_beliefs[newPos] += self.beliefs[oldPos] * prob
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        for i in range(self.numParticles):
            self.particles.append(self.allPositions[i % len(self.legalPositions)])
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.particles:
            ...
            # Sample a new position from this distribution and add it to the new particles list
            newParticle = newPosDist.sample()
            newParticles.append(newParticle)

        # Update the particles with the new list of particles
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        for i in range(self.numParticles):
            self.particles.append(possible_combinations[i % len(possible_combinations)])
