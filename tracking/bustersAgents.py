...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        likely_positions = []
        for dist in ghosts_distribution:
            likely_position = max(
                dist, key=dist.get
            )  # Get the position with the highest probability
            likely_positions.append(likely_position)

        # Step 2: Determine the closest ghost
        closest_distance = float("inf")
        closest_position = None
        for pos in likely_positions:
            distance = self.distancer.getDistance(gameState.getPacmanPosition(), pos)
            if distance < closest_distance:
                closest_distance = distance
                closest_position = pos

        # Step 3: Choose the best action to move towards the closest ghost
        best_action = None
        min_distance = float("inf")
        for action in gameState.getLegalPacmanActions():
            next_position = Actions.getSuccessor(gameState.getPacmanPosition(), action)
            distance_to_ghost = self.distancer.getDistance(
                next_position, closest_position
            )
